...
class Settings(BaseSettings):
    ...
    def validate_database_url(cls, v: str) -> str:
        ...
        try:
            ...
            if not password:
                raise ValueError(
                    "데이터베이스 비밀번호(Password)가 정의되지 않았거나 비어 있습니다. 안전한 비밀번호를 설정해야 합니다."
                )
            if password.lower() in (
                ...
                raise ValueError(
                    f"취약한 기본 패스워드('{password}')가 감지되었습니다. 추측하기 어려운 강력한 비밀번호를 설정하십시오."
                )
        except Exception as e:
            if isinstance(e, ValueError):
                raise e
            raise ValueError(
                "DATABASE_URL 포맷 파싱에 실패했습니다. 올바른 DB 연결 정보 형식이어야 합니다."
            )
        return v
    ...
    def validate_secret_key(cls, v: str) -> str:
        ...
        if len(v) < 32:
            raise ValueError(
                "JWT SECRET_KEY는 보안 강화를 위해 최소 32자 이상이어야 합니다."
            )
        if v.lower() in ("changeme", "secretkey", "default", "password", "1234567890"):
            raise ValueError(
                "취약한 기본값이 SECRET_KEY로 설정되었습니다. 임의의 강력한 난수 키를 지정하십시오."
            )
        return v
    ...
    # R2 클라우드페어 폴더 경로
    R2_TEMPLATE_PREFIX: str = "template/"  # 템플릿 양식 저장소
