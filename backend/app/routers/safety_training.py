...
def get_safety_trainings(
    ...
    """안전 교육 이수 목록 조회. emp_id 파라미터로 개인 조회 가능 (Member용)."""
...
def get_safety_training(training_id: str, db: Session = Depends(get_db)):
...
def update_safety_training(training_id: str, data: SafetyTrainingCreate, db: Session = Depends(get_db)):
    training = db.get(SafetyTraining, training_id)
    if not training:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="교육 내역을 찾을 수 없습니다.")
...
def delete_safety_training(training_id: str, db: Session = Depends(get_db)):
    training = db.get(SafetyTraining, training_id)
    if not training:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="교육 내역을 찾을 수 없습니다.")
    db.delete(training)
