...
def _find_document_by_path(db: Session, uploader: str, r2_key: str) -> Document | None:
    stmt = select(Document).where(
        and_(
            Document.uploader == uploader,
            Document.file_path == r2_key,
        )
    )
...
def _upsert_document_metadata(
    ...
    *,
    ...
    existing = _find_document_by_path(db, uploader, file_path)
    now = _utc_now()
    ...
    doc = _find_document_by_path(db, uploader, file_path)
    if doc is None:
...
def sync_r2_documents(db: Session, *, uploader: str) -> dict:
    ...
    for prefix in prefixes:
        ...
        for obj in objects:
            ...
            existed = _find_document_by_path(db, uploader, obj["key"]) is not None
